...
for i in range(0,npoly):
    ...
    if not noplot:
        xg = arange(xstart,xstart+(xext+2)*dx,dx)
        yg = arange(ystart,ystart+(yext+1)*dx,dx)

        figure(); pcolormesh(x3,y3,shpxras1)
        ax1 = gca()
    ...
    xexta = x2a - x1a
    yexta = y2a - y1a
    outshpa = (int(ceil(yexta) + 1), int(ceil(xexta) + 1))
    ...
    x3a = trnsfmr1a.xy(0, arange(outshpa[1], dtype=int32))[0]
    y3a = trnsfmr1a.xy(arange(outshpa[0], dtype=int32), 0)[1]

    ixmin = int((xstarta+0.5) - dqv.x[0])
    ixmax = int((xstarta+0.5) - dqv.x[0] + ceil(xexta) + 1)
    iymin = int(dqv.y[0] - ystarta+0.5)
    iymax = int(dqv.y[0] - ystarta+0.5 - ceil(yexta) - 1)
    ...
    if pdout.shape[0] == 0:
        ...
        pdout.loc[fid] = {'VG_ELEV': vgelev, 'VG_FRAC': vgfrac, 'VFNAN': vfnan, 'ATOT_M2': cur1.ATOT_M2,
                          'LS_VGELV': ls_vgelev, 'ELV_ERR': vfelv_err, 'FLG': c1flg, 'ELPSD_T': elapsed_time1}

    print(f'fid: {fid}; vfmnelv: {vfmnelev: .2f}; elapsed time: {elapsed_time1: .1f}\n')
    sys.stdout.flush()
