...
if 'd5' not in locals():
    ...
    # vf1b = vf1b.reindex(index=np.arange(7822,dtype=int64))
...
pdnew = pload('p2n-all.pickle')
pdnew = pdnew.loc[pdnew.ELV_ERR > -10]

# st = time.time()

fid_err_list = list(pdnew.loc[np.abs(pdnew.ELV_ERR) > 1].index)
# fid1 = fid_err_list[3]
fid1 = 24500

######################################################################
######################################################################
# for fid1 in fid_err_list:
...
shpv = vf1.loc[vf1.FID_CMU == fid1].geometry.iloc[0]


fid = cur1.name

elverr = float(pdnew.loc[pdnew.index == fid1].ELV_ERR.iloc[0])
...
# flag any places where the vgelev is greater than the max CoNED values
# in the region, or smaller than the minimum
maxoutq = outq.max()
minoutq = outq.min()
if vgelev > 0:
    if vgelev > maxoutq:
...
if (qvsl1.min().values < 0) & (qvsl1.max().values > 0):
    qvsl1.plot(cmap=cm.seismic)
else:
    qvsl1.plot(cmap=cm.nipy_spectral)

plt.pcolormesh(x3a, y3a, shp1ras1a, alpha=0.3, cmap=cm.cool)
# p4p = gpd.GeoSeries(p4)
# p4p.plot(ax=ax1, alpha=0.6)
plt.title(f'fid: {fid}, in CoNED crs (UTM, Z18N)')

ax2 = plt.gca()
shpv1 = transform(prj1, shpv)
shpv1p = gpd.GeoSeries(shpv1)
shpv1p.plot(ax=ax2, facecolor="None")
...
print(f'lspx area: {lspxarea} ;; vgarea = {vgarea}')
print(f'gradshp: {bgshp} ;; gradpx: {bgpx}')
sname = None

if biggrad:
    print('flag area: -100000')
    sname = f'phase2_plots/err_out/steep_gradients/{fid}_{elverr:0=+5.2f}_{lspxarea/vgarea:.2f}.png'

if ((lspxarea/vgarea) < 0.6):
    print('flag area: -1000000')
    sname = f'phase2_plots/err_out/too_many_nans-low_coverage/{fid}_{elverr:0=+5.2f}_{lspxarea/vgarea:.2f}.png'

if sname is None:
    sname = f'phase2_plots/err_out/{fid}_{elverr:0=+5.2f}_{lspxarea/vgarea:.2f}.png'
# plt.savefig(sname)
